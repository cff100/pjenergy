...
def filtro_especifico(plataforma_representacao: str, coluna_de_filtragem: str, objetos_de_filtragem: Union[Any, list]) -> dd.DataFrame:
...
def filtro_faixa_de_valores(plataforma_representacao: str, coluna_de_filtragem: str,
...
def filtra_estacao(plataforma_representacao: str, estacoes: Union[str, list]) -> dd.DataFrame:
    ...
    df_filtrado = filtro_especifico(plataforma_representacao, cr.DadosVariaveis.ESTACAO_DO_ANO, estacoes )
...
def filtra_ano(plataforma_representacao: str, anos: Union[int, list]) -> dd.DataFrame:
    ...
    df_filtrado = filtro_especifico(plataforma_representacao, cr.DadosVariaveis.ANO, anos )
...
def filtra_mes(plataforma_representacao: str, meses: Union[str, int, list]) -> dd.DataFrame:
    ...
    if ( isinstance(meses, list) and all(isinstance(mes, int) for mes in meses) ) or isinstance(meses, int):
        df_filtrado = filtro_especifico(plataforma_representacao, cr.DadosVariaveis.MES, meses)
    elif ( isinstance(meses, list) and all(isinstance(mes, str) for mes in meses) ) or isinstance(meses, str):
        df_filtrado = filtro_especifico(plataforma_representacao, cr.DadosVariaveis.MES_STR, meses)
    else:
...
def filtra_hora(plataforma_representacao: str, horas: Union[str, int, list]) -> dd.DataFrame:
    ...
    if ( isinstance(horas, list) and all(isinstance(mes, int) for mes in horas) ) or isinstance(horas, int):
        df_filtrado = filtro_especifico(plataforma_representacao, cr.DadosVariaveis.HORA, horas)
    elif ( isinstance(horas, list) and all(isinstance(mes, str) for mes in horas) ) or isinstance(horas, str):
        df_filtrado = filtro_especifico(plataforma_representacao, cr.DadosVariaveis.HORA_STR, horas)
    else:
...
def filtra_velocidade_vento(plataforma_representacao: str, componente: Literal["u","v", "resultante"] = "resultante", 
    ...
    df_filtrado = filtro_faixa_de_valores(plataforma_representacao, coluna_de_filtragem, 
                                          maior_igual_a, menor_igual_a)

    return df_filtrado


def filtra_temperatura(plataforma_representacao: str, 
                            maior_igual_a: Optional[int] = None, menor_igual_a: Optional[int] = None) -> dd.DataFrame:
...
if __name__ == "__main__":
    ...
    print(f'Filtrado por anos: {df_filtrado.compute()}\n')

    df_filtrado = filtra_estacao("p7", ["Outono", "Inverno"])
    print(f'Filtrado por estações: {df_filtrado.compute()}\n')

    df_filtrado = filtra_mes("p7", [8, 4])
    print(f'Filtrado por meses: {df_filtrado.compute()}\n')

    df_filtrado = filtra_hora("p7", [6])
    print(f'Filtrado por horas: {df_filtrado.compute()}\n')

    df_filtrado = filtra_velocidade_vento("p7", menor_igual_a = 14)
    print(f'Filtrado por velocidades do vento: {df_filtrado.compute()}\n')

    #print(df_filtrado["t_C"].compute().min())
    #print(df_filtrado["t_C"].compute().max())

    df_filtrado = filtra_temperatura("p7", maior_igual_a = 25)
    print(f'Filtrado por temperaturas: {df_filtrado.compute()}\n')

    print(f'Tipos: {df_filtrado.dtypes}\n')
