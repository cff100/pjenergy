...
def criacao_grafico(ax, cor, Y, X, Y_smooth, X_smooth, grafico, plataforma, estacao, horario, variavel, componente_velocidade, data, ano):
  ...
  if grafico == 'original':
    ...
    if variavel == 'velocidade':
      if data == None:
        ax.set_title(f'Perfil de Velocidade do Vento - {plataforma} - Est: {estacao} - Ano: {ano} - Dir: {componente_velocidade}')  # Define o título para perfis de velocidade
      else:
      ...
      if data == None:
        ax.set_title(f'Perfil de Temperatura - {plataforma} - Est: {estacao} - Ano: {ano}')  # Define o título para perfis de temperatura
      else:
